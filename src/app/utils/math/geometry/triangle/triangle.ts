...
export class Triangle {
  ...
  contains(pointToTest: Point) {
    const areaSum = this.points.reduce((accumulator, currentPoint, index) => {
      ...
      return (accumulator += newTriangleArea.getArea());
    }, 0);

    return areaSum.toPrecision(6) === this.getArea().toPrecision(6);
  }
   ...
   */
  getArea() {
    const lenghts = this.getSidesLength();

    const semiPerimeter = lenghts.reduce((accumulator, next) => {
      return accumulator + next / 2;
    }, 0);

    const differenceProducts = lenghts.reduce((accumulator, next) => {
      return accumulator * (semiPerimeter - next);
    }, 1);

    return Math.sqrt(semiPerimeter * differenceProducts);
  }
